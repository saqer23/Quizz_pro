...
const CreateQuestionModal = ({
  ...
  const addOption = () => {
    ...
    setOptions([
      ...options,
      { text: currentOption, isCorrect: isCurrentOptionCorrect },
    ]);
  ...
  return (
    <div className="absolute top-[50%] max-w-[480px] mx-auto translate-y-[-50%] flex justify-start p-5 gap-10 flex-col items-center bg-slate-800 shadow-lg shadow-slate-600 rounded-lg border border-slate-600 inset-0 h-max">
      ...
      <form
        ...
        className="w-full max-w-[480px] flex flex-col gap-5"
      >
        <span className="flex flex-col gap-3">
          <label htmlFor="questionText">ادخل السؤال</label>
          <input
            type="text"
        ...
        <span className="flex flex-col gap-3">
          <label htmlFor="options">ادخل الخيارات</label>
          <span className="flex items-center flex-col gap-2">
            <input
              type="text"
              placeholder="اضافة خيار"
            ...
            <span className="flex items-center gap-2 self-start justify-between w-full">
              <span className="space-x-2">
                <label htmlFor="isCorrect">الجواب الصحيح؟</label>
                <input
                  type="checkbox"
                  name="isCorrect"
                  id="isCorrect"
                  ...
                  onChange={() =>
                    setIsCurrentOptionCorrect(!isCurrentOptionCorrect)
                  }
                />
              </span>
              <button
        ...
        <span className="flex flex-col gap-1">
          {options.map((option, index) => (
            <div key={index} className="flex gap-2 items-center">
              <p>{option.text}</p>
              {option.isCorrect && (
                <span className="text-green-500">(صحيح)</span>
              )}
              <button
                type="button"
