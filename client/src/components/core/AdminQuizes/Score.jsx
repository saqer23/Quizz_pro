...
const Score = ({ quiz }) => {
  ...
  return (
    <div className="bg-slate-900 z-[2] w-full rounded-lg py-5 flex flex-col gap-1 text-xl">
      {loading ? (
        <div className="text-center">تحميل...</div>
        ...
        <div className=" border rounded-lg border-slate-600 overflow-hidden">
          ...
          <div className="flex justify-between px-5 py-3">
            <p className="text-green-600">اسم المستخدم</p>
            <p className="text-green-600">النتيجة</p>
          </div>
          {[...scores].reverse().map((score, index) => (
            ...
            >
              ...
              <p>
                <span
                  className={`${
                    score?.score / score.answers.length >= 0.4
                      ? "text-green-500"
                      : "text-red-700"
                  }`}
                >
        ...
        <p className="text-center">لا يوجد بيانات حاليا</p>
      )}
    </div>
  );
};
