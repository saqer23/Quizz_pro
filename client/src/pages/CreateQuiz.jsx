...
const CreateQuiz = () => {
  ...
  return (
    <div className="min-h-[70vh] flex justify-center flex-col items-center gap-10">
      <h3 className="text-3xl underline text-center">اضافة كويز</h3>
      <form
        onSubmit={handleSubmit(submitHandler)}
        className="w-full max-w-[480px] border mx-auto py-10 flex flex-col gap-5 p-10 rounded-lg shadow-lg shadow-blue-400"
      >
        ...
        <span className="flex flex-col gap-2">
          ...
          ></textarea>
          {errors.description && (
            <RequiredError>{errors.description.message}</RequiredError>
          )}
        </span>
        <span className="flex flex-col gap-2">
          <label htmlFor="timer">الوقت (دقائق)</label>
          <input
            type="number"
            placeholder="5.00"
            id="timer"
